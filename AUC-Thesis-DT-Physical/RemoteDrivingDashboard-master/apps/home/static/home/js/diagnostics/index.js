  ...
  rect.selectAll('.day')
    ...
    .append('rect')
      ...
      .attr('title', (d) => {
        ...
        if (!countData) return `No hour on ${date}`;
        else if (!countData || !countData.count) return `<1 hour on ${date}`;
        else if (countData.count === 1) return `1 hours on ${date}`;
        else return `${countData.count} hours on ${date}`;
      })
      .attr('date', (d) => d)
